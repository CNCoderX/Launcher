...
//    }
    ...
    private static View getIconInDirection(CellLayout layout, int i, int delta) {
        final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(layout);
        return findIndexOfIcon(views, i, delta);
    }
    private static View getIconInDirection(CellLayout layout, View v, int delta) {
        final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(layout);
     ...
     */
    private static View getClosestIconOnLine(CellLayout layout, View v, int lineDelta) {
        final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(layout);
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_LEFT:
                if (handleKeyEvent) {
                    ...
                    View newIcon = getIconInDirection(layout, v, -1);
                    if (newIcon != null) {
                        newIcon.requestFocus();
                    } else {
                        if (pageIndex > 0) {
                            newIcon = getIconInDirection(layout, layout.getChildCount(), -1);
                            if (newIcon != null) {
                                newIcon.requestFocus();
                            } else {
            ...
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                if (handleKeyEvent) {
                    ...
                    if (newIcon != null) {
                        ...
                        if (pageIndex < (pageCount - 1)) {
                            newIcon = getIconInDirection(layout, -1, 1);
                            if (newIcon != null) {
                                newIcon.requestFocus();
                            } else {
            ...
            case KeyEvent.KEYCODE_DPAD_UP:
                if (handleKeyEvent) {
                    ...
                    View newIcon = getClosestIconOnLine(layout, v, -1);
                    if (newIcon != null) {
                        newIcon.requestFocus();
                        wasHandled = true;
                    }
            ...
            case KeyEvent.KEYCODE_PAGE_UP:
                if (handleKeyEvent) {
                    ...
                    // if there is no previous page
                    if (pageIndex > 0) {
                        View newIcon = getIconInDirection(layout, -1, 1);
                        if (newIcon != null) {
                            newIcon.requestFocus();
                        } else {
                            // Snap to the previous page
            ...
            case KeyEvent.KEYCODE_PAGE_DOWN:
                if (handleKeyEvent) {
                    ...
                    if (pageIndex < (pageCount - 1)) {
                        ...
                        View newIcon = getIconInDirection(layout, layout.getChildCount(), -1);
                        if (newIcon != null) {
                            newIcon.requestFocus();
                        }
                    }
